...
let bookDetail = document.getElementById("bookdetails");
...
let reviewDetails = document.getElementById("details");
let reviewStatus = document.getElementById("reviewStatus");
let newTitle = document.getElementById("addTitle");
let newAuthor = document.getElementById("addAuthor");
let newPrice = document.getElementById("addPrice");
let newCover = document.getElementById("addCover");

let displayReviews = document.getElementById("showReview");
...
for (book of books){
    ...
    tab.innerHTML +=`<tr>
                        ...
                        <td>${book.author}</td>
                        <td>${book.price}</td>
                        <td><button id="details" onclick="bookDetails(book)">Details</button> <button id="delete" onclick="bookDelete()">Delete</button></td>
                    </tr>`;
...
function bookDetails(book){
    console.log('bookDetails called',book.title);
...
function showBook(book){
    ...
    bookDetail.innerHTML = `Author: ${book.author} <br>
                            Price: ${book.price}<br>
                            Rating: ${book.rating} <br>

                            `
    bookImage.innerHTML = `<img src="${book.cover}">`;


    console.log("inside show review");
    for(let i=0;i<localStorage.length;i++){
        let value = localStorage.getItem(book.title);
        let values = JSON.parse(value);
        displayReviews.innerHTML += `
                                    ${values[0]} rated ${values[1]}<br>
                                    ${values[2]}
                                    `
    }
...
function saveReview(book){
    ...
    const details = reviewDetails.value;
    arr.push(name);
    arr.push(rating);
    arr.push(details);
    ...
    reviewRating.value = '';
    reviewDetails.value = '';
    reviewStatus.innerHTML = `Review Saved`;
...
function saveBook(){
    ...
    newObj.author = newAuthor.value;
    newObj.price = newPrice.value;
    newObj.cover = newCover.value;
    books.push(newObj);
    document.getElementById("bookStatus").innerHTML = "Book added Successfuly";
}

function bookDelete(book){
    console.log("delete button clicked")
    if(books.length){
        console.log("inside if block of delete");
        books.splice(book, 1);
