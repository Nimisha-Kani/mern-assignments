...
class  BookManager{
    ...
    saveReviews(id,reviews){
        ...
        if(this.reviews && this.reviews.length>0){ //if this.books is valid and has length greater than 0
            localStorage.setItem(id, JSON.stringify(this.reviews)); //save all books as a JSON string
            console.log("Review Stored");
            console.log(reviews);  
        }  
    }
    loadReviews(id){
        console.log("inside loadReview")
        let localStoreReviews = localStorage.getItem(id);
        if(!localStoreReviews){
           return undefined;
        }
        try{
        let reviews = JSON.parse(localStoreReviews); //try to convert json data as books
        console.log("inside localreview function",reviews);
        return reviews; //return it
        }catch(err){
            return undefined; //if you fail return undefined.
        }
    ...
    addReview(id,review){
        // this.arrReview.push(review);
        console.log("iddddd inside addReview",id);
        console.log("Review inside addReview manager",review);
        this.reviews.push(review);
        console.log("review array inside manager Addreview",this.reviews);

        this.saveReviews(id,review);
    ...
    getBookById(id){
        console.log("Inside getBook by Id",id)
