...
const AuthorEditor = ({addAuthor, updateAuthor}) => {
    ...
    if(status.type===ActionNames.STATUS_ERROR){
        ...
        if(status.error.response.status===401 || status.error.response.status===403){
            //we need to login again!
            //logout(dispatch);
            console.log('user should log in');
            history.push('/user/signin?error=You need to login to perform this action');
        }
    }
...
export default connect(null,actions)(AuthorEditor);