...
//  import {useDispatch} from 'react-redux'; //replaced with
//+ connect
import {connect} from 'react-redux';

import {getAllAuthors} from '../store/author-action';

const App=({title,getAllAuthors})=>{

    //const dispatch=useDispatch();
    useEffect(()=>{
       //   consolegetAllAuthors(dispatch);
       // dispatch(getAllAuthors());  // - uses useDispatch()  hook
        getAllAuthors();            //+ uses connect() for auto dispatch
    },[]);
...
let actions={
    getAllAuthors   //internally wrapped as ()=> dispatch(getAllAuthors())
}
